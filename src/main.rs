...
fn de_string_to_u64<'de, D>(d: D) -> Result<u64, D::Error> where D: serde::Deserializer<'de> {
...
async fn main_() -> Result<()> {
    ...
    let copy_regions: Vec<_> = resolved_regions.into_iter().filter(|f| f != &initial_region).collect();
    ...
    let resp = ec2_client.register_image(rusoto_ec2::RegisterImageRequest{
        name: ami_name.clone(),
        ...
        ..Default::default()
    }).await.expect("could not register ami");
    let init_ami_id = resp.image_id.unwrap();

    println!("registered ami: region={},id={}", initial_region.name(), init_ami_id);

    for region in &copy_regions {
        let ec2_client = Ec2Client::new(region.clone());
        let resp = ec2_client.copy_image(rusoto_ec2::CopyImageRequest{
            name: ami_name.clone(),
...
async fn resolve_all_regions() -> Result<Vec<rusoto_core::region::Region>> {
    ...
    loop {
        let params = ssm_client.get_parameters_by_path(GetParametersByPathRequest{
            path: "/aws/service/global-infrastructure/services/ec2/regions".to_string(),
            next_token: next_token.clone(),
            ..Default::default()
        }).await?;

        let mut regions = params.parameters.unwrap()
            .into_iter()
